...
The repository has a branch for each video starting point. For example, the branch **02-01b** is used as the starting code for the video *02-01 Components metadata*. You can switch branches using **git** and not have to re-run `npm install` each time since you will remain in the same root folder.
